...
class Client:
    ...
    async def historic_rates(self, product_id, granularity=3600, start=None, stop=None):
        ...
        :param str start: (optional) The start time as a str in ISO 8601 format. 
            This field is optional. If it is set, then stop must be set as well. 
            If neither start nor stop are set, start will default to the time 
            relative to now() that would return 300 results based on the 
            granularity.
        
        :param str stop: (optional) The end time as a str in ISO 8601 format. 
            This field is optional. If it is set then start must be set as well. 
            If it is not set, stop will default to now().
        ...
        :raises ValueError: 
            ...
            * Either start or stop is set but not both
        ...
        if start and not stop:
            raise ValueError("stop must be set if start is.")
            
        if stop and not start:
            raise ValueError("start must be set if stop is.")
